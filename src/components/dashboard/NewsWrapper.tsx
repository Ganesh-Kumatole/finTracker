...
import { fetchFinancialNews } from '@/services/news/newsService';
import { NewsWrapperProps } from '@/types/dashboard';
    ...
    <div className="flex items-center justify-between mb-5">
      ...
      <button
        onClick={async () => {
          setNewsLoading(true);
          try {
            const r = await fetchFinancialNews({ sort: 'relevancy' });
            setNews(r.articles.slice(0, 3));
            setNewsError(null);
          } catch {
            setNewsError('Refresh failed');
          } finally {
      ...
      >
        <span
          className={`material-icons-outlined text-base ${newsLoading ? 'animate-spin' : ''}`}
        >
          refresh
        </span>
        Refresh
